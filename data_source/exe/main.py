...
class Processor(QWidget, Ui_mainframe):
    ...
    def executeCallback(self, result):
        ...
        if code == -1:
            ...
            # 出现了欢迎语
            if welcome.findall(msg):
                self.shutdown()
                self.showMessage(type=0, title="第一次使用，请先按说明执行！", msg=msg)
                self.printLogs(log="第一次使用，由于 streamlit 限制，请先手动双击运行同目录下的 app.bat 文件，运行成功之后即可启动本软件！", color="red")
        else:
